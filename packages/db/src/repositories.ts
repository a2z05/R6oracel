...
export function getAllMaps(db: OracleDb) {
  return db.orm.select().from(schema.maps).all();
}

export function getMapById(db: OracleDb, id: string) {
  return db.orm.select().from(schema.maps).where(eq(schema.maps.id, id)).get();
}

export function insertMap(db: OracleDb, data: schema.MapInsert) {
  return db.orm.insert(schema.maps).values(data).onConflictDoNothing().run();
}

// ─── Rooms ────────────────────────────────────────────────────────────

export function getRoomsByMap(db: OracleDb, mapId: string) {
  return db.orm.select().from(schema.rooms).where(eq(schema.rooms.mapId, mapId)).all();
}
...
export function getRoomById(db: OracleDb, id: string) {
  return db.orm.select().from(schema.rooms).where(eq(schema.rooms.id, id)).get();
}

export function insertRoom(db: OracleDb, data: schema.RoomInsert) {
  return db.orm.insert(schema.rooms).values(data).onConflictDoNothing().run();
}

// ─── Aliases ──────────────────────────────────────────────────────────

export function getAliasesByRoom(db: OracleDb, roomId: string) {
  return db.orm.select().from(schema.roomAliases).where(eq(schema.roomAliases.roomId, roomId)).all();
}

export function getAllAliases(db: OracleDb) {
  return db.orm.select().from(schema.roomAliases).all();
}

export function insertAlias(db: OracleDb, data: schema.RoomAliasInsert) {
  return db.orm.insert(schema.roomAliases).values(data).onConflictDoNothing().run();
}
...
export function insertConnection(db: OracleDb, data: schema.ConnectionInsert) {
  return db.orm.insert(schema.connections).values(data).onConflictDoNothing().run();
}

// ─── Spawn Points ─────────────────────────────────────────────────────

export function getSpawnPointsByMap(db: OracleDb, mapId: string) {
  return db.orm.select().from(schema.spawnPoints).where(eq(schema.spawnPoints.mapId, mapId)).all();
}

export function insertSpawnPoint(db: OracleDb, data: schema.SpawnPointInsert) {
  return db.orm.insert(schema.spawnPoints).values(data).onConflictDoNothing().run();
}
...
export function insertStratCard(db: OracleDb, data: schema.StratCardInsert) {
  return db.orm.insert(schema.stratCards).values(data).onConflictDoNothing().run();
}

// ─── Settings ─────────────────────────────────────────────────────────

export function getSetting(db: OracleDb, key: string) {
  const row = db.orm.select().from(schema.settings).where(eq(schema.settings.key, key)).get();
  if (!row) return null;
  try {
...
export function getAllSettings(db: OracleDb) {
  return db.orm.select().from(schema.settings).all();
}
...
export function getOcrCacheEntry(db: OracleDb, normalized: string) {
  return db.orm.select().from(schema.ocrCache).where(eq(schema.ocrCache.normalized, normalized)).get();
}
...
export function getFavoriteMaps(db: OracleDb) {
  return db.orm.select().from(schema.favorites).all();
}

export function addFavorite(db: OracleDb, mapId: string) {
  return db.orm.insert(schema.favorites).values({ mapId }).onConflictDoNothing().run();
}

export function removeFavorite(db: OracleDb, mapId: string) {
  return db.orm.delete(schema.favorites).where(eq(schema.favorites.mapId, mapId)).run();
}
